...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, pos=None, global_merging=None) -> Tensor:
        ...
        if global_merging is not None and global_merging in merge_num:
            ...
            # 4. VGGT-360 核心：保存注意力图供 Module 3 (3D模型校正) 使用
            self.saved_attn_map = attn_probs.detach()
            
            # 5. 乘以 Value
            x = torch.matmul(attn_probs, v)
        else:
            # 如果没有传入 bias，则走 FastVGGT 原生的高效路径
...
class MemEffAttention(Attention):
    def forward(
        self, x: Tensor, attn_bias=None, pos=None, global_merging=None, structure_bias=None
    ) -> Tensor:
        assert (
        ...
        # 【修改 4】：合并原有的 attn_bias 和新增的 structure_bias
        if structure_bias is not None:
            if attn_bias is None:
                attn_bias = structure_bias
            else:
                attn_bias = structure_bias + attn_bias
